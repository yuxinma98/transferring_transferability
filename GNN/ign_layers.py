...
class layer_2_to_2(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # bias
        mat_diag_bias = torch.eye(inputs.size(3)).unsqueeze(dim=0).unsqueeze(dim=0).to(
            inputs
        ) * self.diag_bias.to(inputs)
...
class layer_2_to_1(nn.Module):
    ...
    def __init__(
        self, input_depth, output_depth, normalization="inf", normalization_val=1.0, device="cpu"
    ):
        super().__init__()
...
class layer_basic(nn.Module):
    ...
    def __init__(
        ...
        # bias
        self.bias = torch.nn.Parameter(torch.zeros(1, self.output_depth, 1, 1)).to(
            device=self.device
        )
    ...
    def forward(self, inputs):
        ...
        ops_out.append(
            torch.cat([sum_of_cols.unsqueeze(dim=2) for i in range(m)], dim=2)
        )  # N x D x m x m
        # w3 - sum rows
        sum_of_rows = torch.sum(inputs, dim=3) / float_dim  # N x D x m
        ops_out.append(
            torch.cat([sum_of_rows.unsqueeze(dim=3) for i in range(m)], dim=3)
        )  # N x D x m x m
        # w4 - sum all
...
def contractions_2_to_2(inputs, dim, normalization="inf", normalization_val=1.0):  # N x D x m x m
    ...
    op5 = torch.diag_embed(
        torch.cat([sum_all.unsqueeze(dim=2) for d in range(dim)], dim=2)
    )  # N x D x m x m
...
def contractions_1_to_2(inputs, dim, normalization="inf", normalization_val=1.0):  # N x D x m x m
    ...
    op2 = torch.diag_embed(
        torch.cat([sum_all for d in range(dim)], dim=2), dim1=2, dim2=3
    )  # N x D x m x m
